// 1.
// Create a variable named firstName and set it equal to your first name. Set another
// variable named lastName to your last name. Join the two together, separated by a
// space, and store the result in a variable named fullName. Log the value of fullName.

const firstName = 'Seph';
const lastName = 'Malt';
...
// 8.
// Create two variables, a = 'a' and b = 'b'. Log a "greater than" comparison between
// the two variables. Reassign the value 'B' to variable b, then compare the two
// variables again, and log the comparison value.
...
// 9.
// Create variables aIndex and vIndex and store the index of the first occurrences of
// letters 'a' and 'v' in the language string. Call the substr method on language with
// aIndex as the first argument, and 4 as the second argument, e.g., language.substr(aIndex, 4).
// Log the return value. Repeat the operation using vIndex as the first argument.
...
// and 'Kids like it too', respectively. Combine the values of the two variables with
// the string ' and ' between them, and store the result in a variable named
...
const compoundSentence = fact1.concat(' and ', fact2.toLowerCase());
...
// result is a number, you can log typeof boxNumber to verify the type of the result.
// Now convert the number back to a String by using the String() function and log
// the typeof of the result to verify it is now a String.
...
If the user appends a ! to his name (e.g., 'Bill!'), the computer should "yell" its
greeting: that is, it should log everything to the console in uppercase. You can
check whether the name ends with a ! using String.prototype.endsWith() (ES6 only).
